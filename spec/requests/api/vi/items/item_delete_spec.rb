...
RSpec.describe 'the item delete endpoint' do
  ...
  it 'deletes the invoice if only one item was present' do
    ...
    @invoice_item_1 = InvoiceItem.create(quantity: 1, unit_price: 100.0, invoice_id: @invoice_1.id,
                                         item_id: @item1.id)
    @invoice_item_2 = InvoiceItem.create(quantity: 1, unit_price: 100.0, invoice_id: @invoice_1.id,
                                         item_id: @item2.id)
    @invoice_item_3 = InvoiceItem.create(quantity: 1, unit_price: 100.0, invoice_id: @invoice_1.id,
                                         item_id: @item3.id)
    @invoice_item_4 = InvoiceItem.create(quantity: 1, unit_price: 100.0, invoice_id: @invoice_2.id,
                                         item_id: @item3.id)
